...
# ============================================================
# ============================================================
# OPTIMIZED VERSION — changes vs. ANOVA_App_NL.R
# 1. Colour fix: all CSS shorthand hex (#888, #555) replaced with
#    6-digit R-compatible equivalents (#888888, #555555) — prevents
#    "invalid RGB specification" crash in ss_decomp_plot.
# 2. clear_feedback_store() rewritten: no longer calls
#    reactiveValuesToList(feedback_store) to discover IDs.
#    Uses a static known-ID list + reactive-isolated group IDs.
#    Avoids accumulating reactive dependencies on every reset.
# 3. Dead renderUI outputs removed: means_detail_feedback,
#    ss_detail_feedback, and anova_table_detail_feedback were
#    computed server-side but had no matching uiOutput() in the UI.
#    Removing them eliminates unnecessary reactive computation.
# ============================================================
...
server <- function(input, output, session) {
  ...
  # OPTIMIZED: output$means_detail_feedback removed — had no matching uiOutput() in UI.
